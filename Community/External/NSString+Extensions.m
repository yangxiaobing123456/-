...
- (NSString *)stringFromSHA256
{
    ...
    CC_SHA256(value, strlen(value), outputBuffer);
...
{
    NSDate *date = [NSDate dateWithTimeIntervalSince1970:millisecond / 1000.f];
...
+ (NSString *)getPastDayWithMillisecond:(long long)millisecond
{
    ...
    NSDateComponents *components = [[NSCalendar currentCalendar] components:NSYearCalendarUnit|NSMonthCalendarUnit|NSDayCalendarUnit|kCFCalendarUnitHour|kCFCalendarUnitMinute|kCFCalendarUnitSecond
                                                                   fromDate:date toDate:[NSDate date] options:0];
    return [NSString stringWithFormat:@"%d", [components day] + 1];
}
