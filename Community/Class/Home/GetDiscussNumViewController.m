...
-(void)requestFinished:(ASIHTTPRequest *)request{
    if (request.tag==100) {
        ...
        if ([result isEqualToString:@"1"])
        {
            ...
            talkTable.frame = CGRectMake(0, 0, 320, table2Height);
...
- (void)pinglunBtnClick:(UIButton *)btn
{
    ...
    pingView.frame = CGRectMake(103,lineHeight+talkTable.frame.origin.y-44-35-44, 180, 39);
    //判断是否点赞
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex==1&&alertView.tag==100) {
        ...
        [self taolunDeleteBtnClick:alertView];
    }
}
...
- (void)deleteBtnClick:(UIButton *)btn
{
    ...
    idstringPing = [[discussListArray objectAtIndex:indexPath.section-1] objectForKey:@"id"];
    NSLog(@"idString===%@",idstringPing);
...
- (void)deleteTalkBtnClick:(UIButton *)btn
{
    ...
    //然后使用indexPathForCell方法，就得到indexPath了~
    NSIndexPath *indexPath = [talkTable indexPathForCell:cell];
    idstringTao = [[[[discussListArray objectAtIndex:indexPath.section-1] objectForKey:@"commentList"] objectAtIndex:indexPath.row-2] objectForKey:@"id"];
...
//    idstring = [[discussListArray objectAtIndex:btn.tag] objectForKey:@"id"];
    NSLog(@"idString===%@",idstringPing);
    
    NSDictionary *user = [[NSDictionary alloc]initWithObjectsAndKeys:idstringPing,@"discussId", nil];
...
- (void)taolunDeleteBtnClick:(UIAlertView *)aler
{
    [pingView removeFromSuperview];

    NSLog(@"idString===%@",idstringTao);
    
    NSDictionary *user = [[NSDictionary alloc]initWithObjectsAndKeys:idstringTao,@"commentId", nil];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    {
        NSArray *commentArray = [[discussListArray objectAtIndex:section-1] objectForKey:@"commentList"];
...
//            PinglunCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
            ...
            CGFloat tentHeight = [cell.content.text sizeWithFont:[UIFont systemFontOfSize:14] constrainedToSize:CGSizeMake(190, 1000) lineBreakMode:NSLineBreakByCharWrapping].height;
            //delType
            ...
            cell.content.frame = CGRectMake(60, 34, 190, tentHeight);
            
            //坐标问题
            cell.pinglunBtn.frame = CGRectMake(265, tentHeight+35, 49, 45);
            cell.pingImage.frame = CGRectMake(291, tentHeight+45, 19, 15);
            cell.imageLine.frame = CGRectMake(0, tentHeight+71.5, 320, 0.5);
...
//            TalkCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
            ...
            NSArray *ommentListArray = [[discussListArray objectAtIndex:indexPath.section-1] objectForKey:@"commentList"];
            
            NSDictionary *dic = [ommentListArray objectAtIndex:indexPath.row-2];
            ...
            CGFloat entHeight = [cell.content.text sizeWithFont:[UIFont systemFontOfSize:14] constrainedToSize:CGSizeMake(200, 1000) lineBreakMode:NSLineBreakByCharWrapping].height;
            
            cell.content.frame = CGRectMake(80, 34, 200, entHeight);
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    }else
    {
        ...
        {
            NSArray *commentArray = [[discussListArray objectAtIndex:indexPath.section-1] objectForKey:@"commentList"];
            NSDictionary *dic = [commentArray objectAtIndex:indexPath.row-2];
