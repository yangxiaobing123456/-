...
//自定义导航栏  社区活动开发
    ...
    UISegmentedControl *seg = [[UISegmentedControl alloc] init];
     //隐藏圆角
    seg.frame = CGRectMake(-4, -1, 326, 40);
  ...
  //隐藏黄线
    UILabel *lineLabely = [[UILabel alloc] initWithFrame:CGRectMake(0, 38, 320, 2)];
    lineLabely.backgroundColor = RGBA(231, 231, 231, 1);
    [self.view addSubview:lineLabely];
    [self homedown];

    
    tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 33, 320, self.view.bounds.size.height-94) style:UITableViewStylePlain];
...
//#warning 自动刷新(一进入程序就下拉刷新)
    ...
    tableView.footerPullToRefreshText = @"上拉可以加载更多数据";
    tableView.footerReleaseToRefreshText = @"松开马上加载更多数据";
