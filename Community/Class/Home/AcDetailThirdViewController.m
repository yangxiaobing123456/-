...
#define contentLabelheight     290
#define Originheight     155

@interface AcDetailThirdViewController ()

@end

@implementation AcDetailThirdViewController
...
- (void)viewDidLoad {
    ...
    myNewScrollview = [[UIScrollView alloc] initWithFrame:self.view.frame];
    myNewScrollview.contentSize = CGSizeMake(320, 600);
//    myNewScrollview.backgroundColor = [UIColor redColor];
    [self.view addSubview:myNewScrollview];
    
    NSArray *array = [[NSBundle mainBundle]loadNibNamed:@"ActDetailThirdView" owner:self options:nil];
    actDview = [array objectAtIndex:0];
    //xib坐标一定要和此处一致，不然控件不响应
    actDview.frame = CGRectMake(0, 0, 320, Originheight);
    [myNewScrollview addSubview:actDview];
    
//    contentLabel = [[UILabel alloc] init];
//    contentLabel.textColor = [UIColor darkGrayColor];
//    [myNewScrollview addSubview:contentLabel];
...
-(void)requestFinished:(ASIHTTPRequest *)request{
    if (request.tag==100) {
        ...
        if ([result isEqualToString:@"1"]) {
            ...
            actDview.titleLabel.text=[detailDic objectForKey:@"title"];
            actDview.addressLabel.text=[detailDic objectForKey:@"address"];
            actDview.timeLabel.text=[detailDic objectForKey:@"time"];
            actDview.telephoneLabel.text=[detailDic objectForKey:@"telephone"];
            ...
            labelHeight = [self heightForString:[[dic objectForKey:@"appNewActivityVo"]objectForKey:@"content"] font:[UIFont systemFontOfSize:16.0f] andWidth:contentLabelheight];
//            contentLabel = [[UILabel alloc] initWithFrame:CGRectMake(15, Originheight, contentLabelheight, labelHeight)];
            ...
            contentLabel.frame = CGRectMake(15, Originheight, contentLabelheight, labelHeight);
            //适配屏幕大小
            if (Originheight+labelHeight>518) {
                myNewScrollview.contentSize = CGSizeMake(320, Originheight+labelHeight+109);
...
- (void)addaTabarView
{
    ...
    {
        //需要继续优化
        tabarView.frame = CGRectMake(0, screenHeight-109, 320, 45);
    }else
    {
...
- (void)addBottomFenxiangView
{
    ...
    NSLog(@"screenHeight===%f",screenHeight);
    NSLog(@"kScreenWidth===%f",kScreenWidth);
//    bottomView.frame = CGRectMake(0, 300, 320, 45);
    if (screenHeight>481)
    {
        //需要继续优化
        bottomView.frame = CGRectMake(0, screenHeight-109, 320, 45);
    }else
    {
